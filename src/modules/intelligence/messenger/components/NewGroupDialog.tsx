...
export default function NewGroupDialog({ open, onClose, onCreated }: Props) {
  ...
  return (
    <div
      className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-start justify-center pt-20 sov-overlay-enter"
      onClick={handleClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-md mx-4 rounded-2xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl overflow-hidden sov-scale-in"
      >
        {/* Header */}
