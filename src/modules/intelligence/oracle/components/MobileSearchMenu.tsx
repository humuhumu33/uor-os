...
export default function MobileSearchMenu({ open, onClose, onAiMode, onIdentity }: Props) {
  ...
  return (
    <CSSPresence
      show={open}
      enterClass="sov-slide-left-in"
      exitClass="sov-slide-left-out"
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 pt-[env(safe-area-inset-top,16px)] pb-4">
        ...
        <button onClick={onClose} className="p-2 -mr-2 text-muted-foreground/50 hover:text-foreground/80 transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <nav className="flex-1 flex flex-col justify-center px-8 -mt-16">
        {NAV_ITEMS.map((item, i) => (
          <button
            ...
            className="flex items-center gap-4 py-4 text-left group sov-stagger-item"
      ...
      <div className="px-8 py-6 pb-[env(safe-area-inset-bottom,24px)] space-y-3">
        {user ? (
          <>
            <button onClick={() => { onIdentity(); onClose(); }} className="flex items-center gap-3 w-full py-2.5 text-left">
              <Shield className="w-4 h-4 text-muted-foreground/40" />
              <span className="text-base text-foreground/70">{profile?.displayName ?? "My Identity"}</span>
            </button>
            <button onClick={handleSignOut} className="flex items-center gap-3 w-full py-2.5 text-left">
              <LogOut className="w-4 h-4 text-muted-foreground/30" />
              <span className="text-base text-muted-foreground/50">Sign Out</span>
            </button>
          </>
        ) : (
          <button onClick={() => { onIdentity(); onClose(); }} className="w-full py-3 rounded-xl border border-white/[0.1] bg-white/[0.04] text-base font-medium text-foreground/80 hover:bg-white/[0.08] transition-colors">
            Sign In
          </button>
        )}
      </div>
