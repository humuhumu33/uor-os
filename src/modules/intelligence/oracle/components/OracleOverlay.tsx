 ...
 * from the bottom of the reader view.
 */
...
export default function OracleOverlay({
  contextLabel, contextContent, open, onClose, onExpandToOracle, onRenderAsPage, immersive,
}: Props) {
  ...
  const send = async () => {
    ...
    const contextMsg: Msg[] = contextLabel
      ? [{ role: "user" as const, content: `I'm currently reading about "${contextLabel}". ${contextContent ? `Here's a brief excerpt:\n\n${contextContent.slice(0, 800)}` : ""}\n\nMy question: ${trimmed}` }]
      ...
      <div className={`relative rounded-t-2xl border-t border-x border-border/20 shadow-[0_-8px_60px_-12px_hsl(0_0%_0%/0.5)] overflow-hidden flex flex-col ${
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {contextLabel && <span className="ml-2 text-xs text-muted-foreground/40">· {contextLabel}</span>}
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            {messages.length > 0 && (
              <button onClick={() => onExpandToOracle(messages)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-muted-foreground/50 hover:text-foreground/70 hover:bg-muted/10 transition-all" title="Expand to full Oracle">
                <Maximize2 className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">Expand</span>
              </button>
            )}
            <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground/40 hover:text-foreground/60 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
          ...
          {!oracleAvailable && messages.length === 0 && (
            <div className="text-center py-6 flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-muted-foreground/40 leading-relaxed max-w-[260px]">
                You're offline. The Oracle needs an internet connection.
              </p>
            </div>
          )}
          {oracleAvailable && messages.length === 0 && (
            <div className="text-center py-8">
              <p className="text-sm text-muted-foreground/35">Ask anything about {contextLabel ? `"${contextLabel}"` : "this content"}…</p>
            </div>
          )}
          {messages.map((msg, i) => (
        ...
        <div className="px-4 pb-[env(safe-area-inset-bottom,12px)] pt-2">
          <div className="flex items-center gap-2 bg-muted/10 border border-border/15 rounded-xl px-3 py-1">
            ...
            />
            <button onClick={send} disabled={!input.trim() || streaming || !oracleAvailable} className="p-2 rounded-lg text-foreground/60 hover:text-foreground/90 transition-all disabled:opacity-20">
