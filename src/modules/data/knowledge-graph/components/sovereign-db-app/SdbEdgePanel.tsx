    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-border space-y-2">
        <input
          ...
          className="w-full px-3 py-2 rounded-md border border-border bg-muted/50 text-[13px] font-mono focus:outline-none focus:ring-2 focus:ring-primary/30 placeholder:text-muted-foreground/50"
        />
...
function EdgeRow({ edge, open, onToggle }: { edge: Hyperedge; open: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-card hover:bg-muted/20 transition-colors">
      ...
      >
        <span className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[11px] font-mono font-medium shrink-0">
          {edge.label}
        </span>
        ...
        <span className="text-muted-foreground/40 text-[11px] font-mono ml-auto truncate max-w-[140px]">
          {edge.id}
        </span>
      ...
      {open && (
        <div className="px-4 pb-3 space-y-2 text-[12px]">
          <div>
            <span className="font-semibold text-muted-foreground text-[11px] uppercase tracking-wider">Nodes</span>
            <div className="flex flex-wrap gap-1 mt-1">
              {edge.nodes.map((n, i) => (
                <span key={i} className="px-2 py-0.5 rounded bg-muted text-foreground font-mono text-[11px]">
                  {n}
                </span>
              ))}
            </div>
          ...
          {propCount > 0 && (
            <div>
              <span className="font-semibold text-muted-foreground text-[11px] uppercase tracking-wider">Properties</span>
              <pre className="mt-1 p-2 rounded bg-muted/50 font-mono text-[11px] text-muted-foreground overflow-auto max-h-40">
